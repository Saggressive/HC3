...
def proc_data(files,mode="train",language="en"):
    ...
    if mode == "train":
        ...
        split = int(len(all_proc_data)*0.9)
        split_data1,split_data2 = all_proc_data[0:split],all_proc_data[split:]
...
def make_ours_train_data(save_folder,language):
    ...
    ours_train_data,ours_val_data = proc_data(ours_files,"train",language=language)
    hc3_train_data,hc3_val_data = proc_data(hc3_files,"train",language=language)
...
def make_ours_test_data(save_folder,language):
    ...
    ours_test_data = proc_data(ours_files,"test",language)
    hc3_test_data = proc_data(hc3_files,"test",language)
...
if __name__=="__main__":
    #v1没划分我们的数据和对方的数据，v2没有做去重，v3去除gpt和人类一致的部分,v4从从训练集划分数据
    save_folder="ours_dataset_v4"
    language="zh"
